...
{
    static public class DefineFuntion
    {
        public static readonly int[] ArrRow = new int[] { 20, 30, 50, 100, 150, 200, 300 };
        ...
        public static readonly string[] Notice = new string[]
        {
             "",
             "Thêm thành công!",
        ...
        private static readonly string[] VietNamChar = new string[] { "aAeEoOuUiIdDyY", "áàạảãâấầậẩẫăắằặẳẵ", "ÁÀẠẢÃÂẤẦẬẨẪĂẮẰẶẲẴ", "ệéèẹẻẽêếềểễ", "ÉÈẸẺẼÊẾỀỆỂỄ", "óòọỏõôốồộổỗơớờợởỡ", "ÓÒỌỎÕÔỐỒỘỔỖƠỚỜỢỞỠ", "úùụủũưứừựửữ", "ÚÙỤỦŨƯỨỪỰỬỮ", "íìịỉĩ", "ÍÌỊỈĨ", "đ", "Đ", "ýỳỵỷỹ", "ÝỲỴỶỸ" };
        ...
        private static string strRegex = @"/!|@|%|–|\^|\*|\\|\(|\)|\+|\=|\<|\>|\?|\/|,|\.|\:|\;|\'|̣|\&|\#|\[|\]|~|$|!|\||\$";
        ...
        public static string Decrypt(dynamic pstrText)
        {
            try
            {
                var str = (pstrText.ToString() + "").Contains("%") ? HttpUtility.UrlDecode(pstrText.ToString()) : pstrText.ToString();
        ...
        public static int UserId()
        {
            ...
            try
            {
                var obj = System.Web.Security.Membership.GetUser();
        ...
        {
            if ((System.IO.File.Exists(Host + fileName)))
            {
                System.IO.File.Delete(Host + fileName);
            }
        }
        ...
        {
            string str = DefineFuntion.Host + slug;
            return str;
        }
        public static string GetSlugHotel(string slug)
        {
            ...
            if (!slug.Contains("/"))
            {
                str = DefineFuntion.Host + "khach-san/" + slug;
            }
            else
            {

                str = DefineFuntion.Host + slug;
            }
            //var db = new Entities();
