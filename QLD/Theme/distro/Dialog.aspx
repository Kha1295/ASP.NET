<%@ Page Title="File Manager" Language="C#" AutoEventWireup="true" CodeBehind="Dialog.aspx.cs" Inherits="FileManager.Dialog" %>
...
<body>
            ...
            <div class="span12 pull-right">
                <ul class="thumbnails ff-items">
                    ...
                    <%  
                        ...
                        foreach (var objF in this.arrLinks)
                        {
    ...
    <script language="javascript" type="text/javascript">
        ...
        function apply(file, type_file) {
            var strstCl = "<% Response.Write(this.stCl); %>";
                <% if (this.objConfig.strFillSelector != "")
                   { %>
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            $("<% Response.Write("." + this.stCl); %>",window.opener.document).val(file);  
            if(strstCl=="imgList")
                window.opener.LoadItemImage(file);
            $("<% Response.Write("img." + this.stCl); %>",window.opener.document).attr("src",file);
        ...
        function apply_link(file,type_file){
                <% if (this.objConfig.strFillSelector != "")
                   { %>
            $(<% Response.Write(this.objConfig.strFillSelector); %>).val(file);
            $("<% Response.Write("input." + this.stCl); %>",window.opener.document).val(file); 
            if(strstCl=="imgList")
                window.opener.LoadItemImage(file);
            <% Response.Write(this.objConfig.strPopupCloseCode); %>
                <% }
